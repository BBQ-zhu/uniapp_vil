    ...
    >
      ...
      >
        <view class="newName blueBtn border5 flexAlignCenter">
          <view>企</view>
          <view class="ml10">业</view>
          <view>客</view>
          <view class="ml10">户</view>
        </view>
        <view class="ml10" style="width: calc(100% - 100px); flex: 1">
          <view class="flexBetween">
            <view class="reTitle textOver1">
              {{ item.entername }}
              <u-tag
                class="ml10"
                :text="item.status"
          ...
          <view class="flexBetween mt5">
            <view class="color2">业务类型：{{ item.type || "暂无" }}</view>
...
export default {
  ...
  methods: {
      ...
      if (index1 == 0) {
        ...
        uni.showModal({
          ...
          success: async (res) => {
            if (res.confirm) {
              _this.$axios
                .post(_this.$api.deleteEnterprise, { _id: _this.list[index]._id })
                .then(async (res) => {
                  if (res.code == 200) {
    ...
    contentClick(index) {
      uni.navigateTo({
        url:
          "/pages/enterprise/enterprise?data=" +
