<template>
	<view>
		<view class="wrapper" v-show="showContrat">
			<view class="handBtn">
        ...
        <button @click="close" class="delBtn">取消</button>
        <button @click="retDraw" class="saveBtn">重写</button>
        <button @click="previewCanvasImg" class="previewCon">预览</button>
			...
			<view class="handRight">
				<view class="handTitle f16">请签名</view>
			</view>
		</view>
    <!-- 签约区 -->
    <view class="page u-content " v-show="!showContrat">
			<!-- 返回按钮 -->
        ...
        <view class="mt10 signBox poRelative">
            客户签名：
            <u-image :src="signBase64" width="120rpx" height="280rpx" shape="square" style="transform: rotate(-90deg);position:absolute;top:-40px;left:120px;"></u-image>
        </view>
				<view class="poRelative">
					<view class="mt10 signBox">公司盖章：铸力金融服务外包有限公司</view>
				  <u-image :src="'/static/icons/1.PNG'" width="220rpx" height="220rpx" shape="square" style="position:absolute;top:0;left:190rpx;opacity:0.6;"></u-image>
				</view>
...
import { pathToBase64, base64ToPath } from 'image-tools'
	export default {
      ...
      //保存当前界面为图片
      async capture(){
				uni.pageScrollTo({ //回到顶部，避免出现顶部空白
					scrollTop:0,
					duration:0
				})
					 ...
					 await uni.uploadFile({
					    ...
					    success: async (uploadFileRes) => {
									...
									if(res.code == 200){
										...
										await this.$axios.post(this.$api.updataContract, this.contractDetails).then(res => {
											uni.navigateTo({
